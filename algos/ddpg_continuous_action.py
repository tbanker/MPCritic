...
class Args:
    ...
    total_timesteps: int = 50001 # 50001
    ...
    q_target_mu = False
    """use mu for calculating q targets as opposed to actor"""
    pretrain = False
...
"""
    ...
    if args.critic_mode == "vanilla":
        ...
        dpcontrol = DPControl(envs, H=10, mu=mu, linear_dynamics=True, V=V, rb=rb, goal_map=goal_map, lr=args.learning_rate, xlim=np.array([envs.observation_space.low,envs.observation_space.high]), ulim=np.concatenate([envs.action_space.low,envs.action_space.high], axis=0)).to(device)
        dpcontrol_target = DPControl(envs, H=10, mu=mu, linear_dynamics=True, V=V, rb=rb, goal_map=goal_map, lr=args.learning_rate, xlim=np.array([envs.observation_space.low,envs.observation_space.high]), ulim=np.concatenate([envs.action_space.low,envs.action_space.high], axis=0)).to(device)
