...
def template_model(symvar_type='MX'):
    ...
    goal = model.set_variable('_tvp', 'goal')
    ...
    model.set_expression(expr_name='track_error', expr = goal - C_b)
    ...
    model.set_rhs('C_a', 10*F*(C_A0 - C_a) -K_1*C_a - K_3*(C_a**2))
    model.set_rhs('C_b', -10*F*C_b + K_1*C_a - K_2*C_b)
    model.set_rhs('T_R', ((K_1*C_a*H_R_ab + K_2*C_b*H_R_bc + K_3*(C_a**2)*H_R_ad)/(-Rou*Cp)) + 10*F*(T_in-T_R) +(((K_w*A_R)*(-T_dif))/(Rou*Cp*V_R)))
    model.set_rhs('T_K', (1000.0*Q_dot + K_w*A_R*(T_dif))/(m_k*Cp_k))
