...
if __name__ == '__main__':
    ...
    # seeds = np.concatenate([seeds[:7], seeds[8:]])
    ...
    exp_keys = ['learn-f', 'learn-K', 'learn-pd-P'] # list(exp_dicts.keys())[:3]
    ...
    leglabel0s = [r'$A,B$', r'$K$', r'$P$'] # [r'$A,B$', r'$K$', r'$P$', r'$P=L^{\top}L + \epsilon I$']
    color0s = ['C0', 'C1', 'C2'] # ['C0', 'C1', 'C2', 'C5']
    ...
    y1bs = [res_dicts[exp_key][key] for key in ['f_rmses_avg', 'K_rmses_avg', 'P_rmses_avg']]
    yerr1bs = [2*res_dicts[exp_key][key] for key in ['f_rmses_std', 'K_rmses_std', 'P_rmses_std']]
    x1bs = [np.arange(batch_limit)] * len(y1bs) # [np.arange(A_lrn.shape[1])] * len(y1bs)
    leglabel1bs = [None]*3 # [None, None, r'$P=L^{\top}L + \epsilon I$']
    color1bs = color0s # color0s[:2] + color0s[2+1:]
    plotdata1b = {'xs':x1bs, 'ys':y1bs, 'yerrs':yerr1bs, 'leglabels':leglabel1bs, 'colors':color1bs}

    fig, axes = plot_losses(plotdata0, plotdata1a, plotdata1b, ylabels=["Individual", "Co-learning"], xlabel='Batch')
